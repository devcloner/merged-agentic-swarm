---
source_file: "tests/test_agentic_orchestrator.py"
type: "code"
community: "SubTask"
location: "L337"
tags:
  - graphify/code
  - graphify/INFERRED
  - community/SubTask
---

# TestApplyWorkerOutputsPaths

## Connections
- [[dot-setup_method()_1]] - `method` [EXTRACTED]
- [[dot-test_relative_path_resolves_and_write_failure_logged()]] - `method` [EXTRACTED]
- [[dot-test_secondary_text_from_response_dict()]] - `method` [EXTRACTED]
- [[EpicTask]] - `uses` [INFERRED]
- [[MultiLayeredAgenticOrchestrator]] - `uses` [INFERRED]
- [[PRDAnalysisResult]] - `uses` [INFERRED]
- [[SubTask]] - `uses` [INFERRED]
- [[TaskPriority]] - `uses` [INFERRED]
- [[test_agentic_orchestrator.py]] - `contains` [EXTRACTED]

#graphify/code #graphify/INFERRED #community/SubTask